...
type Props = { initialCategory?: string; initialPartner?: string };

export function CoupleChallengesSection({ initialCategory, initialPartner }: Props) {
  const { partners, partnerId, setPartnerId, loading } = usePartner(initialPartner);
  ...
  if (!hasMatch) return <MatchLockedEmpty />;
...
function MatchLockedEmpty() {
  return (
    <div className="rounded-3xl border border-dashed border-border bg-card/40 p-10 text-center">
      ...
      <h2 className="font-display text-2xl font-bold">Couple Challenges unlock with a match.</h2>
      <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
        Would-you-rather, values, future goals and readiness packs open the moment you have a mutual match.
      </p>
